...
一个直观比喻：把整个物理内存想象成一块巨大的黑板，伙伴系统就像黑板管理员，负责"按 2 的幂的格子"整页整页地划块、合并；slab 分配器则是黑板管理员之上的"文具管理员"，它预先把每种常用文具（对象）按固定大小放进格子，用坏了就回收备件、直接复用，而不是每次让黑板管理员重新划一块格。方式虽然多了一层，却让"频繁创建小对象"的场景开销大幅下降——这正是本章两个分配器分工的形象化理解。

为什么要如此强调"效率"？因为内核里同一页可能被**无数内核对象共享**：`struct file`、`struct inode`、`struct dentry`、`struct sk_buff`、`task_struct` 等都是高频创建/销毁的对象。如果它们各自都向伙伴系统申请独立页面，页内剩余空间将白白浪费——而 slab 通过"同类型对象集中在一个页面上、按需复用"把这种浪费降到最低。同时，伙伴系统的"物理连续"要求也决定了它不能分配太大或太碎的块，slab 又把请求大小"对齐到常见桶"，二者从不同粒度配合，才支撑起内核整体稳定高效的运行。
...
### 3.10 反碎片化：movable 区与 cgroup 内存隔离

伙伴系统的碎片化问题在长时间运行的高负载服务器上会明显显现：**空闲内存总量充足，但大块（高阶 order）分配失败**，因为空闲小块零散散布，物理相邻的整块不足。内核的应对方案分为几层：

1. **`__GFP_MOVABLE` / ZONE_MOVABLE**：把"可迁移"的页（用户页缓存/匿名页）与"不可迁移"的页（内核对象）区分管理。可迁移页可以通过内存规整（memory compaction）在后台被搬移，从而"挤出"连续大块给高阶分配。
2. **memory compaction（内存规整）**：后台或同步地把 ZONE_MOVABLE 中的页向同一方向搬移，合并出大块；`/proc/sys/vm/compact_memory` 可手动触发。
3. **cgroup 内存限制与回收**：`memory.limit_in_bytes` 约束单个容器/进程组的内存上限，超限触发该 cgroup 内的 reclaim，避免某个失控进程拖垮整机——这也是容器隔离的基石。

理解这层后，"内存明明有空闲却分配失败"的诡异现象（内核日志常见 `order 的 allocation failure`）就不再神秘：那往往是**碎片化导致的连续内存不足**，而不是真正的总量不足。这类问题在接触 `runmq`/`memcg` 主题时还会反复出现，值得从伙伴系统原理出发记住根因。

### 3.11 OOM 评分与保护机制详解

OOM Killer 并非"随机杀一个进程"，而是有一套**可观测、可干预**的评分机制。理解它能避免生产事故：

```text
oom_badness(process)
  = 该进程当前占用内存权重（rumap 约算 RSS + swap 等）
  - 下降因子：root 进程减轻、capable 权能减轻
  / oom_score_adj 影响（-1000 到 +1000，可手动调整）
  → 得分最高的进程被选中 kill
```
...
| 设置 | 作用 |
|------|------|
| `/proc/<pid>/oom_score_adj` | 直接加减该进程的 OOM 分数；`-1000` 表示**永不参与 kill**（如数据库、监控 agent） |
| `/proc/sys/vm/overcommit_memory` | 决定内核是否允许"超额承诺"内存；0=启发式、1=总是允许、2=禁止超卖（按 swap+ram 比例） |
| `sysctl vm.panic_on_oom` | 置 1 时 OOM 不杀进程直接 panic（适合某些 HA 环境宁可重启） |
| cgroup `memory.oom_control` | 容器内可设置 `oom_control.oom_kill_disable=1` 并让任务挂起而非被杀 |

一个常见的生产故事：某机器在高负载瞬间触发 OOM，无辜地把数据库杀了。事后把数据库 `oom_score_adj` 调到 `-800`，把负载均衡、监控设成 `-500`，同时调低 `swap` 倾向，问题即缓解。这类"救火"本质上就是把本文的 OOM 机制从概念落实到参数。需要强调的是：**OOM 只是最后手段，真正治本是减少无法回收的内核/用户内存、治理泄漏与碎片**；把 OOM 当设计前提是不可取的。
